...
import net.marwinka.mysticalcrops.block.entity.AbstractTableEntity;
...
public class OldRitualEntity  extends AbstractTableEntity {
    public int slots;
    public int size = 10;
    public int maxProgress = 600;

    public ItemStack output = this.getStack(5);
    public OldRitualEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.BOTANICAL_RITUAL_TABLE, pos, state,600);
    }
    @Nullable
    @Override
    ...
    {
        if(this.output.isIn(tags.SEEDS) || this.output.isIn(tags.CRAFT_SEEDS)){
            return false;
        }
        else return slot == 5;
    }
    ...
    public void tick(){
        if (!this.world.isClient) {
            ...
            if (recipe.isPresent() && this.output.getCount() == 1) {
                this.progress++;
                if (this.progress >= this.maxProgress) {
                    for(int i = 1; i < this.size(); i++){
                        if(i != 5) this.getStack(i).decrement(1);
                    }
                    this.output = recipe.get().getOutput();
                    DamageItem();
                }
            }
            else {
